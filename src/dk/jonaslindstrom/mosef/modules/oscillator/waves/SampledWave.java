...
import dk.jonaslindstrom.mosef.modules.oscillator.waves.samplefunctions.SampleFunction;
...
public class SampledWave extends SimpleWave {
	...
	public SampledWave(MOSEFSettings settings, SampleFunction function, int samples) {
		super(settings);
		cacheSamplePoints(function, samples);
		cacheSlopes();
	}
	
	private void cacheSamplePoints(SampleFunction function, int samples) {
		this.samplePoints = new Point2D.Float[samples];
		for (int i = 0; i < samples; i++) {
			float x = 1.0f * i / (samples - 1);
			this.samplePoints[i] = new Point2D.Float(x, function.getValue(x));
		}
	}
